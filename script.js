		...
		(function() {
			...
			var runInContext = function runInContext2(context) {
				...
				function debounce(func, wait, options) {
					var lastArgs, lastThis, maxWait, result2, timerId, lastCallTime, lastInvokeTime = 0,
						leading = false,
						maxing = false,
						trailing = true;
					if (typeof func != "function") {
...
function obfuscate(script, chunkSize, shuffle) {
	...
	lines = lines.map((line) => line.trim()).filter((line) => {
		...
		return chunkify(line, chunkSize).map((chunk) => {
...
console.log(obfuscate(text, 4, true));